...
for image_num in range(len(image_lists)):
    ...
    vaid_points = random.sample(vaid_points, 2000)
    print(len(vaid_points))
    max_disp = 128
    ...
    for k in range(0, len(vaid_points)):
        ...
        for res in range(7, 27, 2):
            ...
            right_strip = right_image[y0:y1, target_left_border - half_window:x + half_window + 1]

            res_path = os.path.join(save_root, 'res_' + str(res))
            left_path = os.path.join(res_path,'left_'+str(pair_num) + '.jpg')
            right_path = os.path.join(res_path, 'right_' + str(pair_num) + '.jpg')
            cv2.imwrite(left_path, left_ori)
            cv2.imwrite(right_path, right_strip)
            # 先正样本
            lable.append(int(1))
            #        print(pair_num)
            pair_num = pair_num + 1
